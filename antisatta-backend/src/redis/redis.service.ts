...
export class RedisService implements OnModuleDestroy {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (url && token) {
      ...
      // Dummy fallback to prevent hard crash if variables are missing
      this.client = new Redis({ url: 'https://fake-url.upstash.io', token: 'fake' });
    }
  }

  async onModuleDestroy() {
    // Upstash REST API is stateless, no disconnect needed
  }
  ...
  async zrevrangeWithScores(
    ...
    // Parse the flat array or objects into member/score pairs
