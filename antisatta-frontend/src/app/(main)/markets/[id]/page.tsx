...
export default function MarketDetailsPage() {
  ...
  const { data: myTrades } = useQuery({
    queryKey: ['my-trades', params.id],
    queryFn: async () => {
      const { data } = await apiClient.get(`/predictions/market/${params.id}/me`);
      return data.data;
    },
  ...
  const { data: recentTrades } = useQuery({
    queryKey: ['recent-trades', params.id],
    queryFn: async () => {
      const { data } = await apiClient.get(`/predictions/market/${params.id}`);
      return data.data;
    },
    refetchInterval: 5000,
  });
    ...
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-[340px] shrink-0 flex flex-col gap-6">
          ...
          {/* Your Positions */}
          {isAuthenticated && myTrades && myTrades.length > 0 && (
            <div className="border border-border rounded-lg p-5">
              <h3 className="font-bold text-sm mb-4">Your Positions</h3>
              <div className="space-y-3">
                {myTrades.map((trade: any) => (
                  <div key={trade.id} className="flex justify-between items-center text-sm border-b border-border last:border-0 pb-3 last:pb-0">
                    <div>
                      <div className="font-semibold">{trade.outcome.name}</div>
                      <div className="text-xs text-muted-foreground">{format(new Date(trade.createdAt), 'MMM d, h:mm a')}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold">${trade.stake}</div>
                      <div className="text-xs text-green-600">to win ${trade.reward}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {recentTrades && recentTrades.length > 0 && (
            <div className="border border-border rounded-lg p-5">
              <h3 className="font-bold text-sm mb-4">Recent Trades</h3>
              <div className="space-y-3">
                  ...
                  <div key={trade.id} className="flex justify-between items-center text-sm border-b border-border last:border-0 pb-3 last:pb-0">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-secondary flex items-center justify-center shrink-0">
                        {trade.user?.avatar ? (
                          <img src={trade.user.avatar} className="w-full h-full rounded-full object-cover" />
                        ) : (
                          <span className="text-[10px] font-bold">{trade.user?.username?.charAt(0).toUpperCase()}</span>
                        )}
                      </div>
                      <div className="flex flex-col">
                        <span className="font-semibold text-xs truncate max-w-[100px]">{trade.user?.username}</span>
                        <span className="text-[10px] text-muted-foreground">{format(new Date(trade.createdAt), 'h:mm a')}</span>
                      </div>
                    </div>
                    <div className="text-right flex flex-col items-end">
                      <span className="font-semibold text-xs">{trade.outcome.name}</span>
                      <span className="text-xs font-bold">${trade.stake}</span>
                    </div>
                  </div>
                ))}
              </div>
